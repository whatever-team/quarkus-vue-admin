            ...
            <a-form-item>
              <a-row :gutter="8" style="margin: 0 -4px">
                ...
                <a-col :span="8" style="padding-left: 4px">
                  <img :src="codeSrc" @click="codeTimestamp = new Date().getTime()"/>
                </a-col>
              </a-row>
            </a-form-item>
          </a-tab-pane>
...
export default {
  ...
  computed: {
    ...
    codeSrc() {
      return BASE_URL + '/captcha?t=' + this.codeTimestamp
    }
  },
  mounted() {
    this.codeTimestamp = new Date().getTime()
  },
  methods: {
    ...
    onSubmit (e) {
      ...
      this.form.validateFields((err) => {
        if (!err) {
          ...
          login(name, password, code, this.codeTimestamp + '').then(this.afterLogin)
        }
      })
    },
    afterLogin(res) {
      ...
      if (loginRes.code === 'SUCCESS') {
        ...
        getRoutesConfig().then(result => {
          const routesConfig = result.data.result.data
